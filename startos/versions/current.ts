...
export const current = VersionInfo.of({
  version: '0.11.1:25',
  releaseNotes: {
    en_US: `The Bitcoin Knots (RDTS) Companion has been removed from Select Node. It no longer has a chain of its own: those rules now take effect at the BLAKE2b fork rather than activating separately, so that option and Bitcoin Knots BLAKE2b had become two names for the same destination. An indexer pointed at it now points at Bitcoin Knots BLAKE2b, and rebuilds its index, because the old one describes a chain this can no longer reach.
